...
export async function requestVolumes(search: string, category: Category, orderBy: OrderBy, startIndex: number): Promise<VolumesResponse | undefined> {
  let result = undefined
  const cat = category === 'all' ? '' : `+subject:${category}`
  const response = await fetch(
    `${BOOKS_API_BASE_URL}?q=${search}${cat}&orderBy=${orderBy}&key=${KEY}&maxResults=30&startIndex=${startIndex}`
  )
  if (response.ok) {
